...
bool xDeviceRelayService::OnProxyPacket(xRD_ProxyConnection * Conn, xPacketCommandId CommandId, xPacketRequestId RequestId, const ubyte * Payload, size_t PayloadSize) {
    switch (CommandId) {
        ...
        default:
            DEBUG_LOG("unrecognized protocol %" PRIx32 "", CommandId);
            break;
    }
    return false;
}
...
bool xDeviceRelayService::OnProxyCreateConnection(xRD_ProxyConnection * Conn, const ubyte * Payload, size_t PayloadSize) {
    ...
    DEBUG_LOG("NewConnection: did=%" PRIx64 ", ProxySideConnectionId=%" PRIx64 "", R.RelaySideDeviceId, R.ProxySideConnectionId);
    ...
    DEBUG_LOG("RelaySideContextId=%" PRIx64 ", TargetPort=%u", RCC->RelaySideContextId, (unsigned)RCC->TargetPort);
    ...
    if (R.TargetAddress) {
        ...
        D->CtrlConnection->PostMessage(Cmd_DV_RL_DnsQuery, RCC->RelaySideContextId, DQ);
    } else {
        DEBUG_LOG("Invalid create connection request");
        return false;
    }
...
bool xDeviceRelayService::OnProxyDestroyConnection(xRD_ProxyConnection * Conn, const ubyte * Payload, size_t PayloadSize) {
    ...
    DEBUG_LOG("RelaySizeConnectionId=%" PRIx64 "", R.RelaySideContextId);
    auto RC = RelayConnectionManager.GetConnectionById(R.RelaySideContextId);
    if (!RC) {
        DEBUG_LOG("relay connection not found");
        return true;
    }

    auto RL                = xTR_DestroyConnection();
    RL.DeviceSideContextId = RC->DeviceSideContextId;
    RL.RelaySideContextId  = RC->RelaySideContextId;
...
bool xDeviceRelayService::OnProxyPushData(xRD_ProxyConnection * Conn, const ubyte * Payload, size_t PayloadSize) {
    ...
    DEBUG_LOG("RelaySideContextId=%" PRIx64 ", ProxySideConnectionId=%" PRIx64 ", Data=\n%s", R.RelaySideContextId, R.ProxySideConnectionId, HexShow(R.PayloadView).c_str());
    ...
    if (RC->ProxySideConnectionId != R.ProxySideConnectionId) {
        DEBUG_LOG("Proxy side connection id not match: %" PRIx64 ", %" PRIx64 "", RC->ProxySideConnectionId, R.ProxySideConnectionId);
        return true;
    }

    auto DC = DeviceManager.GetDeviceById(RC->DeviceId);
    if (!DC) {
        DEBUG_LOG("Device connection not found");
        return true;
    }

    auto Push                = xTR_PostData();
    Push.DeviceSideContextId = RC->DeviceSideContextId;
    Push.RelaySideContextId  = RC->RelaySideContextId;
...
bool xDeviceRelayService::OnProxyNotifyConnectionState(const ubyte * Payload, size_t PayloadSize) {
    ...
    auto RCP = RelayConnectionManager.GetConnectionById(N.RelaySideContextId);
    if (!RCP) {
        DEBUG_LOG("relay connection not found");
        return true;
    }
    ...
    DEBUG_LOG("TotalUpload: %" PRIu64 ", TotalDumped: %" PRIu64 "", N.TotalUploadedBytes, N.TotalDumpedBytes);
    auto TN                = xTR_ConnectionStateNotify();
    TN.DeviceSideContextId = RCP->DeviceSideContextId;
    TN.RelaySideContextId  = RCP->RelaySideContextId;
